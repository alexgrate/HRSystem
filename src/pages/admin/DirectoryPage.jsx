...
const DirectoryPage = () => {
  ...
  const [allOffices, setAllOffices] = useState([]); // report-location name lookup + picker
  ...
  const SETUPS = useMemo(() => ({
      ...
      columns: [
        ...
        { header: "Country", render: (r) => r.country },
        {
          ...
          render: (r) => r.headquarter
            ? <span className="rounded bg-brand/10 px-2.5 py-1 text-xs text-brand font-semibold">Headquarters</span>
    ...
    Departments: {
      ...
      columns: [
        ...
        { header: "Parent", render: (r) => allDepartments.find((d) => d.id === r.parent_department_id)?.name || "—" },
        { header: "Head", render: (r) => { const h = allStaff.find((s) => s.id === r.head_employee_id); return h ? getEmployeeName(h, h.email) : "—"; } },
      ...
      fields: [
        ...
        { key: "description", label: "Description", type: "textarea", clearable: true },
        // Parent department (hierarchy) and Department head — the head is
        // load-bearing: it grants appraisal/team-review authority in the
        // Appraisal module. Both were previously only settable via the API.
        // clearable: they can be unset (sends explicit null on edit).
        { key: "parent_department_id", label: "Parent department", type: "select", clearable: true, options: allDepartments.map((d) => ({ value: d.id, label: d.name })) },
        { key: "head_employee_id", label: "Department head", type: "select", clearable: true, options: allStaff.map((s) => ({ value: s.id, label: getEmployeeName(s, s.email) })) },
    ...
    "Pay Grades": {
      ...
      columns: [
        ...
        // Loan eligibility cap: the pay grade is the source of truth for how
        // much staff on it can borrow. Empty = loans blocked until configured.
        { header: "Max loan", render: (r) => {
          const cap = Number(r.max_loan_applicable);
          return Number.isFinite(cap) && cap > 0 ? `₦${cap.toLocaleString()}` : "—";
        } },
        { header: "Status", render: (r) => <StatusBadge active={r.is_active !== false} /> },
      ],
      fields: [
        ...
        { key: "max_loan_applicable", label: "Max Loan Applicable (₦)", type: "number" },
    ...
    Allowances: {
      ...
      // Loan caps moved to Pay Grades — allowances only carry payroll amounts.
      fields: [
    ...
    "Leave Types": {
      ...
      columns: [
        ...
        { header: "Approval", render: (r) => (r.requires_approval ? "Required" : "Auto") },
        {
          header: "Pay Grades",
          render: (r) => {
            const ids = Array.isArray(r.pay_grade_ids) ? r.pay_grade_ids : [];
            if (ids.length === 0) return <span className="text-ink-faint">All grades</span>;
            const names = ids.map((id) => allPayGrades.find((g) => g.id === id)?.name || "?");
      ...
      fields: [
        ...
        { key: "code", label: "Code", type: "text", required: true, placeholder: "e.g. LV_ANNUAL" },
        { key: "benefit_level_id", label: "Benefit Level", type: "select", required: true, options: allBenefitLevels.map((b) => ({ value: b.id, label: b.name })) },
        {
          key: "pay_grade_ids", label: "Eligible Pay Grades", type: "multiselect",
          options: allPayGrades.map((g) => ({ value: g.id, label: g.code ? `${g.name} (${g.code})` : g.name })),
          help: "Leave empty to make this leave type available to every pay grade.",
          emptyText: "No pay grades configured yet — this leave type will be available to everyone.",
        },
  ...
  }), [allDepartments, allBenefitLevels, allPayGrades]);
  ...
  useEffect(() => {
    const fetchGlobalSetups = async () => {
      try {
        const [depts, grades, benefits, roles, groups, offices, staff] = await Promise.all([
        ...
        setAllOffices(Array.isArray(offices) ? offices : offices?.locations || offices?.data || []);
  ...
  // A status filter (or a search) must apply across the WHOLE roster, not just
  // the current page — otherwise "show only Resigned" would silently hide
  // matches on other pages. Both cases load the full roster and drop pagination.
  const employeeFullRoster =
    tab === "Employees" && (!!employeeSearch || statusFilter !== "ALL");
  ...
  useEffect(() => {
    ...
    const fetchTabData = async () => {
      ...
      try {
        if (tab === "Employees") {
          if (employeeFullRoster) {
            // Client-side search/status-filter needs the whole roster — the API
            // has no server-side search/status parameter.
  ...
  // Office display label — matches the backend format (address, state, country
  // + HQ marker). Prefers the server-resolved report_location_name, falling back
  // to the locally-loaded offices list.
  const formatOffice = (o) => {
    if (!o) return null;
    const parts = [o.address, o.state, o.country]
      .map((p) => (p == null ? "" : String(p).trim()))
      .filter(Boolean);
    const label = parts.join(", ");
    if (!label) return null;
    return o.headquarter ? `${label} (HQ)` : label;
  };
  ...
  const filteredData = listData.filter((item) => {
    ...
    // Employee directory: also match Staff ID, Level, and Report Location.
    const staffIdMatch =
      tab === "Employees" && item?.staff_id && String(item.staff_id).toLowerCase().includes(s);
    const levelMatch =
      tab === "Employees" && item?.level && String(item.level).toLowerCase().includes(s);
    const locationMatch =
      tab === "Employees" && officeName(item) !== "—" && officeName(item).toLowerCase().includes(s);
    return (
  ...
  const handleReactivate = async (item) => {
    const ok = await confirm({
      title: `Reactivate ${empName(item)}?`,
      message: "They’ll be able to log in and use the system again.",
      confirmLabel: "Reactivate",
    });
    if (!ok) return;
    try {
      await api.put(`/api/users/${item.id}`, { status: EMPLOYEE_STATUS.ACTIVE });
      setRefreshTick((t) => t + 1);
      toast.success(`${empName(item)} reactivated.`);
    } catch (err) {
      const msg = err?.httpStatus === 403
        ? (err?.error?.message || err?.message || "You don’t have permission to change this status.")
        : (err?.error?.message || err?.message || "Failed to update employee status.");
      toast.error(msg);
    }
  };
          ...
          {tab === "Employees" && (
            ...
            >
              ...
              {EMPLOYEE_STATUS_ORDER.map((s) => (
                <option key={s} value={s}>{EMPLOYEE_STATUS_META[s].label}</option>
              ))}
            </select>
          )}
        </div>
              ...
              <tbody>
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item, i) => {
                    const statusMeta = employeeStatusMeta(item.status);
                    const blocked = isAccessRevoking(item.status);
                    return (
                      <tr
                        key={item.id || i}
                        ...
                        <td className="px-4 py-3 text-right whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                          {(canReadEmployee || canUpdateEmployee) && (
                            <div className="flex items-center justify-end gap-1">
                              {canReadEmployee && (
                                <button onClick={() => setViewEmployee(item)} className="rounded-lg p-1.5 text-ink-faint hover:bg-sunken hover:text-brand" title="View details">
                              ...
                              {blocked ? (
                                <button
                                  ...
                                  className="rounded-lg p-1.5 text-ink-faint hover:bg-emerald-50 hover:text-emerald-600"
                                  title="Reactivate"
                                >
                                ...
                                <button
                                  ...
                                  className="rounded-lg p-1.5 text-ink-faint hover:bg-amber-50 hover:text-amber-600"
                                  title="Change status"
                                >
      ...
      <AnimatePresence>
        ...
        {selectedEmployee && (
          <EmployeeEditModal
            ...
            isSelf={!!currentUser?.id && selectedEmployee?.id === currentUser.id}
        ...
        <AnimatePresence>
          {viewEmployee && (
            <EmployeeDetailsDrawer
              ...
              onEdit={canUpdateEmployee ? (emp) => { setViewEmployee(null); setSelectedEmployee(emp); } : undefined}
            />
          )}
        </AnimatePresence>
...
// Searchable multi-select over {value,label} options; value is an array of ids.
function MultiSelectField({ label, options = [], value = [], onChange, help, emptyText }) {
  ...
  return (
    <div>
      ...
      {help && <p className="mt-0.5 text-[11px] text-ink-faint">{help}</p>}
      {options.length === 0 ? (
        <p className="mt-1.5 rounded-xl border border-dashed border-line p-3 text-xs text-ink-faint">
        ...
        <>
          {options.length > 5 && (
            <input
              ...
              placeholder="Search…"
              className="mt-1.5 h-9 w-full rounded-xl border border-line bg-card px-3 text-sm outline-none focus:border-brand"
            />
          )}
          <div className="mt-1.5 max-h-40 space-y-1 overflow-y-auto rounded-xl border border-line p-2">
            {visible.length === 0 ? (
              <p className="p-1 text-xs text-ink-faint">No matches.</p>
            ) : (
              visible.map((o) => (
                <label key={o.value} className="flex cursor-pointer items-center gap-2 rounded-lg px-1.5 py-1 hover:bg-sunken">
                  <input
                    type="checkbox"
  ...
  const [form, setForm] = useState(() => {
    ...
    for (const f of config.fields) {
      const fallback =
        ...
        : f.type === "doclist" || f.type === "multiselect" ? []
        : (f.default ?? "");
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      for (const f of config.fields) {
        ...
        else if (f.type === "multiselect") raw[f.key] = Array.isArray(v) ? v : [];
      ...
      if (isEdit) {
        for (const f of config.fields || []) {
          const v = raw[f.key];
          if (f.clearable && (v === "" || v === null || v === undefined) && record?.[f.key] != null) {
            payload[f.key] = null;
          }
        }
      }
...
function EmployeeEditModal({ employee, isSelf = false, departments = [], jobRoles = [], payGrades = [], payGroups = [], offices = [], staff = [], onSaved, onClose }) {
  const toast = useToast();
  const confirm = useConfirm();
  const bio = employee?.employee_biodata || employee?.biodata || {};
  const initialStatus = normalizeStatus(employee?.status);
  ...
  const [form, setForm] = useState({
    ...
    start_date: dateOnly(employee?.start_date),
  ...
  const statusEditable = !isSelf;
  const statusChanged = statusEditable && form.status !== initialStatus;

  const handleSubmit = async (e) => {
    e.preventDefault();

    if (statusChanged && isAccessRevoking(form.status)) {
      const copy = STATUS_CONFIRM_COPY[form.status];
      const ok = await confirm({ ...copy, message: `${copy.message}\n\nEmployee: ${[form.firstName, form.lastName].filter(Boolean).join(" ")}`, danger: true });
      if (!ok) return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        // Admin-managed employment fields. End Date is intentionally NOT sent —
        // it is lifecycle-driven (stamped on resign/terminate/retire, cleared on
        // reactivation), never edited by hand. Level and Estimated Gross are
        // derived server-side and are likewise never sent.
        staff_id: nullable(String(form.staff_id).trim()),
        start_date: nullable(form.start_date),
        report_location: nullable(form.report_location),
      ...
      // Only ever send status for another employee; never for self.
      if (statusChanged) {
        payload.status = form.status;
        if (statusReason.trim()) payload.status_change_reason = statusReason.trim();
      }
      const saved = await api.put(`/api/users/${employee.id}`, payload);
      const appliedStatus = normalizeStatus(saved?.status ?? form.status);
      if (statusChanged && appliedStatus === form.status) {
        toast.success(`Profile updated · status set to ${employeeStatusMeta(form.status).label}.`);
      } else {
      ...
      // Surface the backend's message and map status codes to clear guidance.
      const status = err?.httpStatus;
      const backendMsg = err?.error?.message || err?.message;
      let msg = backendMsg || "Error updating employee profile.";
      if (status === 403) msg = backendMsg || "You don’t have permission to make this change.";
      else if (status === 401) msg = "Your session has expired. Please sign in again.";
      else if (status === 404) msg = "This employee no longer exists.";
      else if (status === 409) msg = backendMsg || "This change conflicts with the current state.";
      else if (status === 422) msg = backendMsg || "Some values are invalid. Please review and try again.";
      toast.error(msg);
    } finally {
      setSaving(false);
    }
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
          <div className="flex-1 overflow-y-auto px-6 py-5 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input value={form.staff_id} onChange={(e) => set("staff_id", e.target.value)} className={inputCls} placeholder="e.g. EMP-00123" />
            </div>
            <div>
              <label className={labelCls}>Report location</label>
              <select value={form.report_location} onChange={(e) => set("report_location", e.target.value)} className={inputCls}>
                <option value="">— None —</option>
                {offices.map((o) => (
                  <option key={o.id} value={o.id}>
                    {[o.address, o.state, o.country].filter(Boolean).join(", ")}{o.headquarter ? " (HQ)" : ""}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <input
                value={employee?.end_date ? dateOnly(employee.end_date) : "While employed"}
                disabled
                title="Set automatically when the employee resigns, is terminated, or retires; cleared on reactivation."
                className={`${inputCls} bg-sunken text-ink-muted cursor-not-allowed`}
              />
            </div>
            <div>
              ...
              <input
                ...
                disabled
                title="Derived from the employee's pay grade."
                className={`${inputCls} bg-sunken text-ink-muted cursor-not-allowed`}
              />
            </div>
            <div>
            ...
            <div>
              ...
              <select
                ...
                aria-disabled={!statusEditable}
                title={statusEditable ? undefined : "You cannot change your own account status."}
                className={`${inputCls} ${statusEditable ? "" : "cursor-not-allowed bg-sunken text-ink-muted"}`}
              >
                {EMPLOYEE_STATUS_ORDER.map((s) => (
                  <option key={s} value={s}>{EMPLOYEE_STATUS_META[s].label}</option>
                ))}
              ...
              {statusEditable ? (
                <p className="mt-1 text-[11px] text-ink-faint">
                  Controls system access. Inactive, Suspended and Terminated sign the employee out and block login.
                </p>
              ) : (
            ...
            {statusChanged && isAccessRevoking(form.status) && (
              <div className="sm:col-span-2">
                <label className={labelCls}>Reason (optional)</label>
                <input
                  value={statusReason}
                  onChange={(e) => setStatusReason(e.target.value)}
                  placeholder="e.g. End of contract, disciplinary review…"
                  className={inputCls}
                />
              </div>
            )}
