...
export const setupService = {
  ...
  deleteOffice: (id) => api.delete(`/api/office-locations/${id}`, { data: {} }),
  ...
  deleteDepartment: (id) => api.delete(`/api/departments/${id}`, { data: {} }),
  ...
  deleteBenefitLevel: (id) => api.delete(`/api/benefit-levels/${id}`, { data: {} }),
  ...
  deletePayGrade: (id) => api.delete(`/api/pay-grades/${id}`, { data: {} }),
  ...
  deletePayGroup: (id) => api.delete(`/api/pay-groups/${id}`, { data: {} }),
  ...
  deleteBenefitLevelAllowance: (id) => api.delete(`/api/setups/benefit-level-allowances/${id}`, { data: {} }),

  getWorkflows: () => api.get('/api/setups/approval-workflows'),
  // Full approval-request detail: workflow, ordered steps, stage position, and
  // the action history (approver name, decision, comment, timestamp). Powers the
  // Approvals Inbox timeline drawer. Authorized server-side to admin / requester /
  // designated approver only.
  getApprovalRequestDetail: (requestId) =>
    api.get(`/api/setups/approval-requests/${requestId}`),
  ...
  deleteWorkflow: (id) => api.delete(`/api/setups/approval-workflows/${id}`, { data: {} }),

  getLeaveTypes: () => api.get('/api/setups/leave-types'),
  // Types the current employee can request (own pay grade + unrestricted).
  getEligibleLeaveTypes: () => api.get('/api/setups/leave-types/eligible'),
  createLeaveType: (data) => api.post('/api/setups/leave-types', data),
  updateLeaveType: (id, data) => api.put(`/api/setups/leave-types/${id}`, data),
  deleteLeaveType: (id) => api.delete(`/api/setups/leave-types/${id}`, { data: {} }),
